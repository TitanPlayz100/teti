...
export class Board {
    ...
    moveMinos(coords, dir, size, value = false) {
        ...
        newcoords.forEach((c, idx) => value ? this.addValue(c, valTable[idx]) : this.setValue(c, valTable[idx]))
        spawnOverlay()
    }

    setComboBoard(start) {
        this.boardState.forEach((row, y) => row.forEach((col, x) => {
            if (x < 3 || x > 6) this.addMinos('S G', [[x, y]], [0, 0])
        }))
        if (start) {
            this.addMinos('S G', [[3, 0], [3, 1], [4, 1]], [0, 0]);
            displayShadow()
        }
    }
}